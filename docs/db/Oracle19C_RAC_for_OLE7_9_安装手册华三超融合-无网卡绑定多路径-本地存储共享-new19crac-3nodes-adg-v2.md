...
#内核对rp_filter取 conf/all 与 conf/<网卡> 两者的最大值,因此私网网卡显式设2即生效
#本环境私网网卡为eth1(其他环境可能为eno8等,以 ip a 实际名称为准);iscsi存储网卡建议同样设置
...
#sqlplus portaluser/Oracle2023#Portal@172.18.13.176:1521/s_portal
#sqlplus onecodeuser/ConeDe2839#CoeN@172.18.13.176:1521/s_onecode

#sqlplus system/Oracle2023#Sys@172.18.13.176:1521/s_dataassets

[oracle@rac02 ~]$ sqlplus pdbadmin/DSApdb2023#ADb@172.18.13.176:1521/s_dataassets
...
expdp test1/test1@172.18.13.176:1521/s_dataassets schemas=test1 directory=expdir dumpfile=test1_20220314.dmp logfile=test1_20220314.log cluster=n
#导出全库
expdp pdbadmin/pdbadmin@172.18.13.176:1521/s_dataassets full=y directory=expdir dumpfile=test1_20220314.dmp logfile=test1_20220314.log cluster=n

##impdp
impdp est1/test1@172.18.13.176:1521/s_dataassets schemas=test1 directory=expdir dumpfile=test1_20220314.dmp logfile=test1_20220314.log cluster=n

##impdp--remap
impdp  est1/test1@172.18.13.176:1521/s_dataassets schemas=test1 directory=expdir dumpfile=test1_20220314.dmp logfile=test1_20220314.log cluster=n   remap_schema=test2:test1 remap_tablespace=test2:test1  logfile=test01.log cluster=n

#expdp-12.2.0.1.0
expdp test1/test1@172.18.13.176:1521/s_dataassets schemas=test1 directory=expdir dumpfile=test1_20220314.dmp logfile=test1_20220314.log cluster=n compression=data_only version=12.2.0.1.0
=========================================
...
sqlplus pdbadmin/DSApdb2023#ADb@172.18.13.172:1521/s_dataassets


impdp pdbadmin/DSApdb2023#ADb@172.18.13.172:1521/s_dataassets full=y directory=expdir dumpfile=xydb_expdp_ds_20240301.dmp  logfile=ds_20240301.log
 
 
sqlplus portaluser/Oracle2023#Portal@172.18.13.176:1521/s_portal

impdp portaluser/Oracle2023#Portal@172.18.13.172:1521/s_portal full=y directory=expdir dumpfile=xydb_expdp_portal_20240301.dmp  logfile=portal_20240301.log

[oracle@k8s-19rac01 expdir]$ impdp pdbadmin/DSApdb2023#ADb@172.18.13.172:1521/s_dataassets full=y directory=expdir dumpfile=xydb_expdp_ds_20240301.dmp  logfile=ds_20240301.log
...
expdp pdbadmin/pdbadmin@172.18.13.176:1521/s_dataassets full=y directory=expdir dumpfile=$dmpfile logfile=$logfile cluster=n 
...
#!!!本环境共享存储是单台iSCSI虚拟机(单点),OCR备份必须定期复制到集群之外(store节点或NFS)
...
20 1 * * 1 scp /home/grid/ocrbak/ocr_export_*.ocr root@k8s-19rac-store:/backup/ocr/ >> /home/grid/ocrbak/ocrbak.log 2>&1
...
#OLR损坏恢复(仅故障节点)
crsctl stop crs -f
/u01/app/19.0.0/grid/bin/ocrconfig -local -restore /u01/app/19.0.0/grid/cdata/k8s-19rac01/backup_xxx.olr
crsctl start crs
```
...
create diskgroup OCR normal redundancy
  disk '/dev/asm-ocr01','/dev/asm-ocr02','/dev/asm-ocr03'
...
打补丁顺序:
k8s-19rac01 GI---> k8s-19rac02 GI ---> k8s-19rac01 DB ---> k8s-19rac02 DB

打补丁前可以先备份grid/oracle目录---k8s-19rac01/k8s-19rac02 用户root执行
cd  /u01/app/19.0.0/
...
#验证静态监听可连(主库节点一上测试)
sqlplus sys/Oracle2023#Sys@xydbadg as sysdba
...
```bash
su - oracle
rman target sys/Oracle2023#Sys@xydb auxiliary sys/Oracle2023#Sys@xydbadg log=/home/oracle/dup_xydbadg.log
...
```bash
su - oracle
dgmgrl sys/Oracle2023#Sys@xydb
...
#### 12.8.1.Switchover(计划内主备互换,零数据丢失)
...
#回切
dgmgrl sys/Oracle2023#Sys@xydbadg
DGMGRL> switchover to xydb;
```

#### 12.8.2.Failover(主库整体故障,应急切换)
...
```bash
#每日(可纳入巡检脚本,异常告警)
dgmgrl sys/Oracle2023#Sys@xydb "show configuration" | grep -E "SUCCESS|WARNING|ERROR"
```
...
#每季度:执行一次12.8.1 switchover演练并回切,验证容灾真实可用
