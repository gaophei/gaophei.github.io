...
public ip            211.70.1.191                       211.70.1.192
private ip           3.3.3.191                              3.3.3.192
vip                  211.70.1.186                       211.70.1.187
scan ip              211.70.1.188
```
#### 1.3.2.网卡配置
...
##### 1.3.2.1.网卡eth0走业务网
```
[root@stuora1 network-scripts]# cat ifcfg-ens18
TYPE="Ethernet"
PROXY_METHOD="none"
BROWSER_ONLY="no"
BOOTPROTO="none"
...
eth0: flags=4163<UP,BROADCAST,RUNNING,MULTICAST>  mtu 1500
        inet 211.70.1.191  netmask 255.255.255.0  broadcast 211.70.1.255
...
Wired connection 1  52d60506-6843-3d91-a55b-3c11adc115a8  ethernet  eth1
...
eth1: flags=4163<UP,BROADCAST,RUNNING,MULTICAST>  mtu 1500
        inet 3.3.3.19191  netmask 255.255.255.0  broadcast 3.3.3.19255
...
eth0: flags=4163<UP,BROADCAST,RUNNING,MULTICAST>  mtu 1500
        inet 211.70.1.192  netmask 255.255.255.0  broadcast 211.70.1.255
...
eth1: flags=4163<UP,BROADCAST,RUNNING,MULTICAST>  mtu 1500
        inet 3.3.3.19192  netmask 255.255.255.0  broadcast 3.3.3.19255
...
default via 3.3.3.191 dev eth1 proto static metric 101
3.3.3.0/24 dev eth1 proto kernel scope link src 3.3.3.19191 metric 101
211.70.1.0/24 dev eth0 proto kernel scope link src 211.70.1.191 metric 100
[root@stuora1 network-scripts]# route -n
Kernel IP routing table
Destination     Gateway         Genmask         Flags Metric Ref    Use Iface
0.0.0.0         211.70.1.254    0.0.0.0         UG    100    0        0 eth0
0.0.0.0         3.3.3.191         0.0.0.0         UG    101    0        0 eth1
...
gateway (3.3.3.191) at <incomplete> on eth1
? (3.3.3.19192) at fe:fc:fe:9c:90:bc [ether] on eth1
? (3.3.3.19192) at fe:fc:fe:9c:90:bc [ether] on eth1
[root@stuora1 network-scripts]#

[root@studata2 network-scripts]# arp -a
gateway (211.70.1.254) at 44:82:e5:e0:7d:0c [ether] on eth0
? (3.3.3.19191) at fe:fc:fe:8b:59:ac [ether] on eth1
? (3.3.3.19191) at fe:fc:fe:8b:59:ac [ether] on eth1
...
cat >> /etc/hosts <<EOF
#public ip eth0
211.70.1.191 stuora1
211.70.1.192 stuora2
#vip
211.70.1.186 stuora1-vip
211.70.1.187 stuora2-vip
#private ip eth1
3.3.3.191 stuora1-prv
3.3.3.192 stuora2-prv
#scan ip
211.70.1.188 rac-scan
...
#显示如下
```
[root@stuora1 network-scripts]# lsblk
...
[root@stuora1 network-scripts]# ls -1cv /dev/sd* | grep -v [0-9] | while read disk; do  echo -n "$disk " ; /usr/lib/udev/scsi_id -g -u -d $disk ; done
...
PING stuora2 (211.70.1.192) 56(84) bytes of data.
64 bytes from stuora2 (211.70.1.192): icmp_seq=1 ttl=64 time=0.128 ms
^C
...
[grid@stuora1 ~]$ ssh stuora2 cat ~/.ssh/id_rsa.pub >>~/.ssh/authorized_keys
The authenticity of host '[stuora2]:23231 ([211.70.1.192]:23231)' can't be established.
...
Warning: Permanently added '[stuora2]:23231,[211.70.1.192]:23231' (ECDSA) to the list of known hosts.
...
stuora2 eth0:211.70.1.192
stuora1 eth0:211.70.1.191

Interfaces found on subnet "3.3.3.0" that are likely candidates for VIP are:
stuora2 eth1:3.3.3.192
stuora1 eth1:3.3.3.191
...
--->eth0:211.70.1.0:Public,eth1:3.3.3.0:Private
--->oracle ASM
...
PRVG-1101 : SCAN name "rac-scan" failed to resolve
  ...
  ------------  ------------------------  ------------------------  ----------
  rac-scan      211.70.1.188              failed                    NIS Entry

ERROR:
PRVF-4657 : Name resolution setup check for "rac-scan" (IP address: 211.70.1.188) failed
...
#比如
sqlplus test/test@211.70.1.188:1521/xydb
```
#日志
```
[root@stuora1 ~]# su - grid
...
Listening Endpoints Summary...
  ...
  (DESCRIPTION=(ADDRESS=(PROTOCOL=tcp)(HOST=211.70.1.191)(PORT=1521)))
  (DESCRIPTION=(ADDRESS=(PROTOCOL=tcp)(HOST=211.70.1.186)(PORT=1521)))
Services Summary...
